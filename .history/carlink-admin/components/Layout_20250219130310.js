...
export default function Layout({ children }) {
    ...
    // Custom cursor states (only used on login page)
    const [cursorPosition, setCursorPosition] = useState({ x: 0, y: 0 });
    const [cursorVariant, setCursorVariant] = useState("default");
    const cursorRef = useRef(null);

    // Only set up cursor effects when not logged in
    useEffect(() => {
        // Don't run cursor effects if user is logged in
        if (session) return;
        ...
        const handleMouseUp = () => {
            // Check what element we're currently hovering over
            const elementUnderCursor = document.elementFromPoint(cursorPosition.x, cursorPosition.y);
            if (elementUnderCursor && 
                (elementUnderCursor.tagName === 'BUTTON' || elementUnderCursor.closest('button'))) {
        ...
        const handleMouseEnterInput = () => setCursorVariant("input");
        const handleMouseLeaveInput = () => setCursorVariant("default");

        window.addEventListener('mousemove', handleMouseMove);
        window.addEventListener('mousedown', handleMouseDown);
        window.addEventListener('mouseup', handleMouseUp);
        
        // Apply cursor styles to buttons and inputs
        ...
        // Set body cursor to none (only on login page)
        ...
        if (document.readyState === 'complete') {
            ...
            window.addEventListener('DOMContentLoaded', applyEventListeners);
        }
        ...
        return () => {
            ...
            window.removeEventListener('DOMContentLoaded', applyEventListeners);
    ...
    // Get cursor style based on variant
    const getCursorStyle = () => {
        const baseStyle = {
            ...
            transition: 'width 0.2s, height 0.2s, background-color 0.2s, transform 0.1s, border 0.2s',
        };
        ...
        switch(cursorVariant) {
            ...
            case "clicking":
                return {
                    ...
                    borderRadius: '50%',
                    transform: `translate(${cursorPosition.x - 18}px, ${cursorPosition.y - 18}px) scale(0.8)`,
                };
            case "input":
                return {
                    ...baseStyle,
                    ...
                    borderRadius: '4px',
                    transform: `translate(${cursorPosition.x - 4}px, ${cursorPosition.y - 13}px)`,
                };
            default:
                return {
                    ...baseStyle,
    ...
    if (!session) {
        return (
            <div className="min-h-screen bg-gray-50 flex items-center justify-center p-4">
                {/* Custom cursor - only rendered on login page */}
                <div ref={cursorRef} style={getCursorStyle()}></div>
                
                <div className="login-container w-full max-w-5xl bg-white rounded-3xl shadow-xl overflow-hidden flex pt-4 pb-4 pr-4">
                    {/* Login Form Section */}
    ...
    // Return normal dashboard with no custom cursor when logged in
    return (
