...
export default function Layout({ children }) {
    ...
    const [isVisibleCursor, setIsVisibleCursor] = useState(true);
    ...
    useEffect(() => {
        ...
        const handleMouseEnterLoginContainer = (e) => {
            if (e.target.closest('.login-container')) {
                setIsVisibleCursor(false);
            }
        };

        const handleMouseLeaveLoginContainer = (e) => {
            if (!e.relatedTarget || !e.relatedTarget.closest('.login-container')) {
        ...
        // Add event listeners to the login container
        const loginContainer = document.querySelector('.login-container');
        if (loginContainer) {
            loginContainer.addEventListener('mouseenter', handleMouseEnterLoginContainer);
            loginContainer.addEventListener('mouseleave', handleMouseLeaveLoginContainer);
        }

        return () => {
            window.removeEventListener('mousemove', handleMouseMove);
            if (loginContainer) {
                loginContainer.removeEventListener('mouseenter', handleMouseEnterLoginContainer);
                loginContainer.removeEventListener('mouseleave', handleMouseLeaveLoginContainer);
            }
        };
    }, []);
    ...
    const customCursorStyle = {
        ...
        opacity: isVisibleCursor ? 1 : 0,
        mixBlendMode: 'difference',
    };

    // Custom cursor style for body
    useEffect(() => {
        if (isVisibleCursor) {
