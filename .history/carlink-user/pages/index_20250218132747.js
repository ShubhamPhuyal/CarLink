...
export default function HomePage({ FeaturedProduct, newProducts, newParts, newCars }) {
  return (
    <div>
      <Header />
...
export async function getServerSideProps() {
  ...
  const allProducts = await Category.findOne({ name: "Tesla" });
  let newCars = [];
  if (partsCategory) {
    newCars = await Product.find({ category: partsCategory._id }, null, { sort: { '_id': -1 }, limit: 4 });
  }
  ...
  return {
    props: {
      ...
      //newProducts: JSON.parse(JSON.stringify(allProducts)),
      newProducts: JSON.parse(JSON.stringify(newCars)), // Keeping original newProducts
