...
export default function Featured({ product }) {
  ...
  const scrollStep = 20; // Scroll step in pixels
  const scrollTargetRef = useRef(null);

  const handleScroll = () => {
    const targetPosition = scrollTargetRef.current.getBoundingClientRect().top + window.scrollY;
    ...
    const animateScroll = (timestamp) => {
      ...
      const scrollY = Math.min(progress / scrollDuration * (targetPosition - window.scrollY), targetPosition);
  ...
  return (
    <Section>
      ...
      <div ref={scrollTargetRef} style={{ height: "800px", marginTop: "2rem" }}>
        {/* You can replace this with your target content */}
