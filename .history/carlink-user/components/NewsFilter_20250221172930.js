...
// Styled components (as defined in your original code)
...
const Select = styled.select`
  width: 160px;
  padding: 10px 12px;
  padding-right: 32px; /* Make room for the arrow icon */
  border-radius: 6px;
...
export default function FilterSection({ products, setFilteredProducts }) {
  ...
  // Unique types and brands from products
  ...
  const handleSearchChange = (e) => {
    ...
    filterProducts(); // Filter results in real-time
  };

  // Handle button clicks
  const handleTypeClick = (type) => {
    setSelectedType(type === selectedType ? "" : type);
    filterProducts(); // Filter results when type changes
  };
