...
export default function FilterSection({ products, setFilteredProducts }) {
  ...
  // Get all unique property names (keys) across products
  ...
  propertyNames.forEach((property) => {
    filterOptions[property] = [...new Set(products.map(p => p.properties?.[property]).filter(Boolean))];
  });
  ...
  const handleFilterChange = (property, value) => {
    ...
    Object.entries(updatedFilters).forEach(([key, val]) => {
      if (val) {
        filtered = filtered.filter(product => product.properties?.[key] === val);
      }
    });
