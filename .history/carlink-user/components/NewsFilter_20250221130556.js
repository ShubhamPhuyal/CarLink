  ...
  const handleSearchKeyPress = (e) => {
    if (e.key === 'Enter') {
      const searchFiltered = products.filter(product =>
        (product.title && product.title.toLowerCase().includes(searchTerm.toLowerCase())) || // Check if title exists
        product.properties?.some(prop => prop.Type?.toLowerCase().includes(searchTerm.toLowerCase())) || // Check if Type exists
        product.properties?.some(prop => prop.Brand?.toLowerCase().includes(searchTerm.toLowerCase())) // Check if Brand exists
      );
  ...
  return (
    <FilterContainer>
      ...
      <SearchInput 
        ...
        onKeyDown={handleSearchKeyPress} // Use onKeyDown instead of onKeyPress
      />
