...
export default function CarMarketplaceNews({ news, setFilteredNews }) {
  ...
  const categories = [...new Set(news.map((article) => article.Type).filter(Boolean))];
  const brands = [...new Set(news.map((article) => article.Brand).filter(Boolean))];
  ...
  const applyFilters = () => {
    ...
    if (selectedCategory) {
      filteredNews = filteredNews.filter(article => article.Type === selectedCategory);
    }

    if (selectedBrand) {
      filteredNews = filteredNews.filter(article => article.Brand === selectedBrand);
    }
      ...
      <Center>
        <ContentWrapper>
          ...
          <FilterWrapper>
            ...
            <button onClick={applyFilters} style={{ padding: "10px 16px", borderRadius: "6px", border: "none", backgroundColor: "#3b82f6", color: "white", cursor: "pointer" }}>
              Apply Filters
            </button>
          </FilterWrapper>
        </ContentWrapper>
